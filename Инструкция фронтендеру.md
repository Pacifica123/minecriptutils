...
- PR должен затрагивать только `src/visualizer/*` и файлы рядом с выходным HTML.
- Приложить краткую демонстрацию (GIF или 2 скрина) для новой UI-фичи.
- Добавить unit-test на core, если PR меняет формат узлов/рёбер (не рекомендуется).

---

Если нужно, скопируй этот файл как `src/visualizer/README.md` или вставь раздел в основной `README.md`. Я уже подготовил минимальную реализацию undo/экспорт в `src/visualizer/templates/custom.js` — фронтендеру хватит этого задела, чтобы развивать интерфейс дальше.
